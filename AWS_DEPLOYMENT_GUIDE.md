...
**⚠️ 중요: AWS Amplify UI가 업데이트되어 환경 변수 위치가 변경되었을 수 있습니다.**
...
3. 브랜치 상세 페이지에서:
   ...
   - 또는 **"빌드 설정"** (Build settings) → 환경 변수 섹션

#### 방법 2: 빌드 설정에서

1. **왼쪽 사이드바** → **"호스팅"** 클릭
2. **`main` 브랜치** → **"작업"** (Actions) → **"빌드 설정 편집"** (Edit build settings)

...
test.describe('P0: ポイント関連クリティカルテスト', () => {
  ...
  test('P0-2: 画像作成 → 生成成功 → ポイント減少（失敗時は減らない）', async () => {
    ...
    await loginUser(page, testUser.email, testUser.password);
    await page.waitForURL(/\/MyPage|\//);
    await page.screenshot({ path: 'test-results/P0-2-step1-login.png', fullPage: true });

    // 2. 初期ポイント確認
    const initialPoints = await getPointsFromAPI(page);
    expect(initialPoints.total).toBeGreaterThan(0);
    console.log(`[P0-2] 初期ポイント: ${initialPoints.total}`);

    // 3. キャラクター作成ページに移動
    await page.goto(`${BASE_URL}/characters/create`);
    await page.waitForLoadState('domcontentloaded', { timeout: 30000 }).catch(() => {});
    await page.waitForTimeout(3000);
    await page.screenshot({ path: 'test-results/P0-2-step2-create-page.png', fullPage: true });

    // 4. 画像生成機能を探す
    // タブを探す（複数の方法で）
    console.log('[P0-2] 画像タブを探しています...');
    
    // タブを探す（character-form-tab クラスで）
    const tabs = page.locator('button.character-form-tab');
    const tabCount = await tabs.count();
    console.log(`[P0-2] タブ数: ${tabCount}`);
    
    // タブのテキストを確認
    for (let i = 0; i < tabCount; i++) {
      const tab = tabs.nth(i);
      const text = await tab.textContent();
      console.log(`[P0-2] タブ ${i}: "${text}"`);
    }
    
    // 画像タブ（インデックス 1）を直接クリック
    if (tabCount > 1) {
      const imageTab = tabs.nth(1); // インデックス 1 = 画像タブ
      const tabText = await imageTab.textContent();
      console.log(`[P0-2] 画像タブをクリックします: "${tabText}"`);
      
      // タブをクリック（JavaScriptで直接クリック、可視性チェックをスキップ）
      try {
        // まず可視性を確認せずにJavaScriptでクリック
        await imageTab.evaluate((el: HTMLElement) => {
          el.click();
        });
        console.log('[P0-2] ✅ タブをクリックしました（JavaScriptクリック）');
      } catch (e) {
        console.log('[P0-2] JavaScriptクリックが失敗、通常のクリックを試みます...');
        try {
          await imageTab.click({ timeout: 5000, force: true });
          console.log('[P0-2] ✅ タブをクリックしました（forceクリック）');
        } catch (e2) {
          console.log('[P0-2] すべてのクリック方法が失敗しました。ページをスクロールして再試行...');
          // ページをスクロールしてから再試行
          ...
          console.log('[P0-2] ✅ タブをクリックしました（スクロール後JavaScriptクリック）');
        }
      }
      ...
      // step 1のコンテンツが表示されているか確認（複数の方法で）
      let hasImageSection = false;
      
      // 方法1: キャラクター画像 テキストで確認
      const imageSectionText1 = page.locator('text=/キャラクター画像/i').first();
      hasImageSection = await imageSectionText1.isVisible({ timeout: 5000 }).catch(() => false);
      
      // 方法2: 画像アップロード テキストで確認
      if (!hasImageSection) {
        const imageSectionText2 = page.locator('text=/画像アップロード/i').first();
        hasImageSection = await imageSectionText2.isVisible({ timeout: 5000 }).catch(() => false);
      }
      
      // 方法3: step 1のコンテンツが表示されているか（display: block）確認
      if (!hasImageSection) {
        const step1Content = page.locator('[style*="display: block"]').filter({ hasText: /画像|アップロード/i }).first();
        hasImageSection = await step1Content.isVisible({ timeout: 5000 }).catch(() => false);
      }
      
      console.log(`[P0-2] 画像セクション表示: ${hasImageSection}`);
      
      if (!hasImageSection) {
        console.log('[P0-2] ⚠️ 画像セクションが表示されていません。再度タブをクリックします...');
        // もう一度クリック（force: true で）
        await imageTab.click({ force: true });
        await page.waitForTimeout(3000);
        
        // 再度確認
        const imageSectionText = page.locator('text=/キャラクター画像|画像アップロード/i').first();
        hasImageSection = await imageSectionText.isVisible({ timeout: 5000 }).catch(() => false);
        console.log(`[P0-2] 画像セクション表示（再確認）: ${hasImageSection}`);
      }
      
      await page.screenshot({ path: 'test-results/P0-2-step3-image-tab-clicked.png', fullPage: true });
    } else {
      console.log('[P0-2] ⚠️ タブが見つかりません');
      await page.screenshot({ path: 'test-results/P0-2-step3-no-tab.png', fullPage: true });
    }
    
    // "AI生成 (1P/1枚)" ボタン 検索（複数の方法で）
    console.log('[P0-2] AI生成ボタンを探しています...');
    
    // まず、画像セクションが表示されているか確認
    const imageSectionText = page.locator('text=/キャラクター画像|画像アップロード/i').first();
    const hasImageSection = await imageSectionText.isVisible({ timeout: 10000 }).catch(() => false);
    console.log(`[P0-2] 画像セクション表示: ${hasImageSection}`);
    
    if (!hasImageSection) {
      console.log('[P0-2] ⚠️ 画像セクションが表示されていません。ページをスクロールして確認します...');
      await page.evaluate(() => window.scrollTo(0, 0));
      await page.waitForTimeout(1000);
      await page.screenshot({ path: 'test-results/P0-2-step3-5-scroll-top.png', fullPage: true });
    }
    
    // 方法1: テキストで探す（より広範囲に）
    let imageGenerateButton = page.getByRole('button', { name: /AI生成/i }).first();
    let hasImageButton = await imageGenerateButton.isVisible({ timeout: 5000 }).catch(() => false);
    
    // 方法2: Sparkles アイコンを含むボタンを探す
    if (!hasImageButton) {
      imageGenerateButton = page.locator('button').filter({ hasText: /AI生成|1P\/1枚|1P|生成/i }).first();
      hasImageButton = await imageGenerateButton.isVisible({ timeout: 5000 }).catch(() => false);
    }
    
    // 方法3: すべてのボタンから探す（より詳細に）
    if (!hasImageButton) {
      const allButtons = page.locator('button');
      const buttonCount = await allButtons.count();
      console.log(`[P0-2] 全ボタン数（再検索）: ${buttonCount}`);
      
      for (let i = 0; i < Math.min(buttonCount, 50); i++) {
        const btn = allButtons.nth(i);
        const text = await btn.textContent().catch(() => '');
        const visible = await btn.isVisible({ timeout: 1000 }).catch(() => false);
        if (text && visible && (text.includes('AI生成') || text.includes('1P') || text.includes('生成'))) {
          console.log(`[P0-2] 候補ボタン (インデックス ${i}): "${text}" (表示: ${visible})`);
          imageGenerateButton = btn;
          hasImageButton = visible;
          if (hasImageButton) {
            console.log(`[P0-2] ✅ AI生成ボタン発見 (インデックス ${i}): "${text}"`);
            break;
          }
        }
      }
    ...
    // 方法4: 画像セクション内で探す
    if (!hasImageButton && hasImageSection) {
      const imageSection = imageSectionText.locator('..').locator('..').first();
      const buttonsInSection = imageSection.locator('button');
      const buttonCount = await buttonsInSection.count();
      console.log(`[P0-2] 画像セクション内のボタン数: ${buttonCount}`);
      
      for (let i = 0; i < buttonCount; i++) {
        const btn = buttonsInSection.nth(i);
        const text = await btn.textContent().catch(() => '');
        const visible = await btn.isVisible({ timeout: 1000 }).catch(() => false);
        if (text && visible && (text.includes('AI生成') || text.includes('1P'))) {
          imageGenerateButton = btn;
          hasImageButton = visible;
          console.log(`[P0-2] ✅ AI生成ボタン発見 (セクション内, インデックス ${i}): "${text}"`);
          break;
        }
      }
    }
    ...
    if (!hasImageButton) {
      await page.screenshot({ path: 'test-results/P0-2-step4-button-not-found.png', fullPage: true });
      const pageText = await page.textContent('body').catch(() => '');
      console.log(`[P0-2] ページ内容（最初の1000文字）: ${pageText.substring(0, 1000)}`);

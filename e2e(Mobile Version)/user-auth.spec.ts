...
test.describe('ユーザー観点: 認証・アカウント管理', () => {
  ...
  test('1-1-5: ログアウト', async ({ page }) => {
    ...
    try {
      ...
      // suspended状態の場合はスキップ
      if (error?.message?.includes('停止') || error?.message?.includes('suspended') || page.url().includes('/suspended')) {
        test.skip(true, 'アカウントが停止状態のため、ログアウトテストをスキップします。');
        return;
      }
      ...
      try {
        ...
        // suspended状態の場合はスキップ
        if (retryError?.message?.includes('停止') || retryError?.message?.includes('suspended') || page.url().includes('/suspended')) {
          test.skip(true, 'アカウントが停止状態のため、ログアウトテストをスキップします。');
          return;
        }
        throw retryError;
      ...
      if (hasEmailInput) {
        ...
        if (hasLoginButton) {
          ...
          if (!hasLogoutButtonAfter) {
            ...
            // ログアウトボタンがまだ表示されている場合は、ログインページにいることを確認
            const currentUrl = page.url();
            if (currentUrl.includes('/login')) {
              console.log('[1-1-5] ✅ ログアウト成功 (ログインページにリダイレクト)');
            } else {
              // ログインページにいない場合は失敗
              throw new Error('ログアウトが完了していません。ログアウトボタンがまだ表示されており、ログインページにもリダイレクトされていません。');
            }
          }
        }
      }
  ...
  test('1-1-6: パスワード変更', async ({ page }) => {
    ...
    try {
      ...
      if (error?.isSuspended || error?.message?.includes('停止') || error?.message?.includes('suspended') || page.url().includes('/suspended')) {
        test.skip(true, 'アカウントが停止状態のため、パスワード変更テストをスキップします。');
        return;
      }
      throw error;
    }

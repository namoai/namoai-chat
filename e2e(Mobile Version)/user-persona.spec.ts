...
test.describe('ユーザー観点: ペルソナ機能', () => {
  ...
  test('1-10-4: ペルソナ削除', async ({ page }) => {
    ...
    if (hasConfirmButton) {
      ...
      // ダイアログがない場合は、ペルソナが削除されたことを確認
      await page.waitForTimeout(2000);
      const personaList = page.locator('[data-testid="persona-item"], .persona-item, [class*="persona"]');
      const personaCount = await personaList.count();
      const deletedPersona = page.locator(`text="${personaNickname}"`);
      const isDeleted = !(await deletedPersona.isVisible({ timeout: 2000 }).catch(() => false));
      
      if (isDeleted) {
        console.log('[1-10-4] ✅ ペルソナ削除確認完了（ペルソナが一覧から消えました）');
      } else {
        throw new Error('ペルソナ削除が確認できませんでした。確認ダイアログも見つからず、ペルソナも一覧から消えていません。');
      }
    }

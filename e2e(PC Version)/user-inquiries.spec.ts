...
test.describe('ユーザー観点: お問い合わせ・通報機能', () => {
  ...
  test('1-9-1: お問い合わせ作成', async ({ page }) => {
    ...
    console.log(`[1-9-1] モーダルに __STRING_DOUBLE_0__ テキスト あり: ${hasInquiryTypeText}`);
    ...
    for (let i = 0; i < allSelects.length; i++) {
      ...
      console.log(`[1-9-1] select ${i} オプション: ${options.join(__STRING_SINGLE_0__)}`);
    ...
    if (!categorySelect) {
      ...
      for (let i = 0; i < modalSelects.length; i++) {
        ...
        const options = await select.locator('option').allTextContents();
        console.log(`[1-9-1] モーダル 内部 select ${i} オプション: ${options.join(__STRING_SINGLE_0__)}`);
    ...
    console.log(`[1-9-1] 問い合わせ タイプ select オプション: ${options.join(__STRING_SINGLE_0__)}`);
    ...
    console.log(`[1-9-1] が 入力 後 React 状態: __STRING_DOUBLE_0__`);
    ...
    if (!isEnabled) {
      ...
      if (categoryValue && titleValue && contentValue) {
        ...
        if (!reactState.title) {
          ...
          console.log(`[1-9-1] が 再入力 後 状態: __STRING_DOUBLE_0__`);
        }
  ...
  test('1-9-2: キャラクター通報', async ({ page }) => {
    ...
    if (!reportMenuItem) {
      ...
      for (let i = 0; i < Math.min(redButtons.length, 10); i++) {
        ...
        console.log(`[1-9-2] 間 ボタン ${i + 1}: text=__STRING_DOUBLE_0__, visible=${visible}`);
        
        if (text.includes('通報') && visible && !reportMenuItem) {
          reportMenuItem = redButtons[i];
          console.log(`[1-9-2] ✅ 間 ボタンで 通知 ボタン 発見: __STRING_DOUBLE_0__`);
          break;
        }
      }
    ...
    for (let i = 0; i < Math.min(options.length, 10); i++) {
      ...
      console.log(`[1-9-2] オプション ${i + 1}: text=__STRING_DOUBLE_0__, value=__STRING_DOUBLE_1__`);
    }
    ...
    if (options.length > 1) {
      ...
      console.log(`[1-9-2] 選択された 値: __STRING_DOUBLE_0__`);
      ...
      if (selectedValue !== '不適切なコンテンツ') {
        ...
        console.log(`[1-9-2] 再選択された 値: __STRING_DOUBLE_0__`);
      }
    ...
    if (hasConfirmModal) {
      ...
      if (hasError) {
        ...
        console.log(`[1-9-2] 現在 select 値: __STRING_DOUBLE_0__`);

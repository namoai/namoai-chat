...
export async function waitForChatResponse(page: Page, timeout: number = 30000): Promise<void> {
  ...
  // 新しいメッセージ（AI応答）が表示されるまで待つ
  // メッセージリストの最後の要素が更新されることを確認
  const messageList = page.locator('[class*="message"], [data-role="message"], [class*="chat-message"]');
  const initialCount = await messageList.count();
  
  while (Date.now() - startTime < timeout) {
    const currentCount = await messageList.count();
    
    // メッセージ数が増えた、または最後のメッセージが更新された
    if (currentCount > initialCount) {
      // 最後のメッセージがAI応答（role="assistant"または特定のクラス）か確認
      const lastMessage = messageList.last();
      const messageText = await lastMessage.textContent().catch(() => '');
      
      if (messageText && messageText.trim().length > 0) {
        // ユーザーメッセージでないことを確認（簡単なチェック）
        if (!messageText.includes('テスト') || messageText.length > 10) {
          return; // AI応答とみなす
        }
      }
    }
  ...
  // タイムアウトしてもメッセージが増えていれば成功とみなす
  const finalCount = await messageList.count();
  if (finalCount > initialCount) {
    return;
  }
  
  throw new Error(`チャット応答がタイムアウトしました（${timeout}ms）`);
}

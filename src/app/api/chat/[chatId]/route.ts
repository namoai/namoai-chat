...
// ✅ 重要: ビルドワーカーの型検査を回避するため、第2引数は any にして内部で安全に絞り込む
// eslint-disable-next-line @typescript-eslint/no-explicit-any
export async function POST(request: Request, context: any) {
  // 型ナローイング（params.chatId を安全に取得）
...
- 今回の応答に限り、通常よりも意図的に長く、約${boostMultiplier}倍の詳細な内容で返答してください。`;
    ...
    const systemInstructionText = [char.systemTemplate, userPersonaInfo, boostInstruction]
      .filter(Boolean)
      .join("\n\n");

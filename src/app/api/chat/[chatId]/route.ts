...
export async function POST(request: Request, context: any) {
  ...
  try {
    ...
    const [persona, history] = await Promise.all([
      ...
      prisma.chat_message.findMany({ where: { chatId: chatId, isActive: true, id: { not: userMessage.id } }, orderBy: { createdAt: "asc" } }),
    ]);
    ...
    const systemInstructionText = [guidelineOverrideInstruction, systemTemplate, formattingInstruction, imageInstruction, userPersonaInfo, lorebookInfo, boostInstruction].filter(Boolean).join("\n\n");
    ...
    const stream = new ReadableStream({
      async start(controller) {
        ...
        // Server-Sent Events (SSE) 形式でデータを送信するヘルパー関数
        ...
        try {
          ...
          // 4. 保存済みのAIメッセージ(ID含む)をクライアントに送信
          sendEvent('ai-message-saved', { modelMessage });

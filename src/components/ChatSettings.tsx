...
// 設定項目コンポーネント
const SettingItem = ({ icon, label, onClick, hasSwitch = false, switchState = false, onSwitchChange = () => {} }: SettingItemProps) => (
  <button onClick={onClick} className="flex items-center justify-between w-full p-3 text-left hover:bg-gray-700 rounded-md transition-colors">
...
const NoteModal = ({ note, onSave, onClose }: NoteModalProps) => {
  ...
  const handleSave = async () => {
    if (isOverLimit) {
      alert(`ユーザーノートは${MAX_NOTE_LENGTH}文字以内で入力してください。`);
      return;
    }
    setIsSaving(true);
  ...
  const renderMainView = () => (
    <>
      ...
      <div className="p-2 space-y-1">
        ...
        {/* ▼▼▼【ここから修正】ペルソナページへのリンクにクエリパラメータを追加 ▼▼▼ */}
        <SettingItem 
          icon={<FileText size={20} />} 
          label="ペルソナ" 
          onClick={() => { 
            if (characterId && chatId) {
              window.location.href = `/persona/list?fromChat=true&characterId=${characterId}&chatId=${chatId}`; 
            } else {
              // もし情報がなければ、通常のリンクにフォールバック
              window.location.href = '/persona/list';
            }
          }} 
        />
        {/* ▲▲▲【ここまで修正】▲▲▲ */}

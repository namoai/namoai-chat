...
### 1番: 管理者アカウントのアクセス制限とパスワード管理

- [x] **管理者2FA（2要素認証）** - Google Authenticator等のTOTP対応 ✅（基本構造実装済み、要テスト）
- [x] **管理者ログイン失敗10回でアカウントロック** - 失敗回数カウント機能 ✅（一般ユーザーと同様に実装済み）
- [ ] **IP制限 OR 基本認証** - 管理ページへのアクセス制限（2FAがあれば優先度低い、要検討）

### 2番: データディレクトリの露出対策

- [x] **重要ファイルがpublicディレクトリに保存されていないことを確認** - Vercel/Netlify環境で自動対応
- [x] **アップロードファイルの種類・拡張子制限** - 既に実装済み（PNG/JPEG/WebP/GIFのみ、5MB制限、MIME検証）

### 3番: Webアプリケーションの脆弱性

- [x] **SQLインジェクション対策** - Prisma ORM使用
- [x] **XSS対策** - React標準エスケープ、sanitize-html使用
- [x] **入力値検証（Zod）** - 既に実装済み（registerSchema等）
- [ ] **定期的な脆弱性評価・侵入テスト** - ドキュメント化のみ（フレームワーク更新 + npm audit定期実行）

### 4番: マルウェア対策

- [x] **ウイルス対策ソフト導入・運用** - 開発端末に標準対策、クラウド事業者のセキュリティ機能活用（ドキュメント化）

### 5番: カードテスト対策

- [ ] **EMV 3-D Secure有効化** - Stripe Dashboard設定確認
- [x] **Stripe Radar使用** - Stripe標準機能（ドキュメント化）
- [x] **有効性チェック回数制限** - Stripe側で自動制御（ドキュメント化）

### 6番: ログインセキュリティ（各カテゴリで1つ以上）

**ユーザー登録時:**
- [x] **メールアドレス認証** - 登録後メール認証リンク送信、認証完了までログイン不可 ✅

**ユーザーログイン時:**
- [x] **パスワード試行回数制限** - 10回失敗で15分間ロック ✅

**ユーザーアカウント変更時:**
- [x] **再認証** - パスワード変更時に現在のパスワード再入力（既に実装済み）✅

### 確認事項（実装不要）

- [ ] **.env等の重要ファイルがpublicディレクトリに露出していないか確認**
- [x] **ORM使用（SQLインジェクション防止）** - Prisma使用済み
- [x] **XSS防止（React標準エスケープ）** - 実装済み
- [x] **CSRFトークン/SameSiteクッキー** - NextAuthで自動対応
